...
import urllib.parse
import webbrowser
import threading # Tambahan untuk backup email
import smtplib # Tambahan untuk backup email
from email.mime.multipart import MIMEMultipart # Tambahan untuk backup email
from email.mime.base import MIMEBase # Tambahan untuk backup email
from email import encoders # Tambahan untuk backup email
...
class MainScreen(Screen):
    ...
    def setup_ui(self):
        ...
        self.spin_rekap_h = Spinner(text=self.sel_hari, values=[str(i).zfill(2) for i in range(1, 31)], font_size='20sp', size_hint_x=0.15, background_color=(0.9, 0.95, 1, 1), color=WARNA_TEKS_UTAMA, background_normal='')
    ...
    def save_data(self):
        try:
            ...
            # Fitur Simpan Otomatis ke Email
            self.backup_ke_email()
        except Exception as e:
            print(f"Gagal Simpan: {e}")

    # Fungsi Baru: Backup ke Email (Background Thread)
    ...
    def konfirmasi_restore(self, filename):
        ...
        pesan = (
            f"[b][color=FF0000]PERINGATAN![/color][/b]\n\n"
            f"Anda akan memulihkan data dari file:\n[i]{filename}[/i]\n\n"
            f"Data saat ini akan [b]DITIMPA TOTAL[/b].\n"
            f"Apakah Anda yakin?"
        )
        ...
        if os.path.exists(self.file_data):
            try:
                with open(self.file_data, "r") as f:
                    d = json.load(f); self.anggota = d.get("anggota", {}); self.transaksi = d.get("transaksi", []); self.p3k = d.get("p3k", []); self.config = d.get("config", {"nom_iuran": "10000"})
            except: pass
    ...
    def buka_riwayat_transaksi(self, _):
        ...
        self.fr_h = Spinner(text="Semua", values=["Semua"] + [str(i).zfill(2) for i in range(1, 31)], font_size='16sp'); self.fr_b = Spinner(text="Semua", values=["Semua"] + [str(i).zfill(2) for i in range(1, 13)], font_size='16sp'); self.fr_t = Spinner(text=datetime.now().strftime("%Y"), values=["Semua"] + [str(i) for i in range(2025, 2101)], font_size='16sp')
    ...
    def buka_p3k(self, _):
        ...
        tp.add_widget(h_row); self.ly_p3k = GridLayout(cols=1, spacing=1, size_hint_y=None); self.ly_p3k.bind(minimum_height=self.ly_p3k.setter('height')); vs = ScrollView(do_scroll_x=False); vs.add_widget(self.ly_p3k); tp.add_widget(vs); h_sc.add_widget(tp); con.add_widget(con) # Perbaikan hierarki layout p3k
        # Sebenarnya con.add_widget(h_sc) sudah benar di kode asli. Lanjut:
    ...
    def simpan_p3k(self, _):
        ...
        if n: self.p3k.append({"id": str(datetime.now().timestamp()), "tgl": datetime.now().strftime("%d-%m-%Y"), "nama": n, "jenis": self.p3k_jenis.text, "in": int(self.p3k_in.text or 0), "out": int(self.p3k_out.text or 0), "exp": self.p3k_exp.text or "-"}); self.save_data(); self.update_tabel_p3k("")
    ...
    def hapus_agt_manual(self, _):
        if self.in_n_m.text in self.anggota: del self.anggota[self.in_n_m.text]; self.save_data(); self.update_agt_list("")
    ...
    def proses_bayar(self, sw=False):
        ...
        if sw and n in self.anggota:
            ...
            msg = (
                ...
                f"━━━━━━━━━━━━━━━━━━\n\n"
                f"Jazakumullah Khairan Katsiran. Semoga sedekah iuran ini menjadi wasilah pembersih harta, pembuka pintu rezeki, dan pemberi keberkahan bagi keluarga Bapak/Ibu di dunia maupun akhirat. Amin ya Rabbal 'Alamin. 🤲 🕋\n\n"
                f"Salam takzim,\n"
                f"*Bendahara Kas Blangking*"
            )
    ...
    def kirim_wa(self, n, w, s, md):
        ...
        if md == "besok":
            msg = (
                f"Assalamu’alaikum Bapak/Ibu *{n}*. ✨\n\n"
                f"Semoga cahaya keberkahan senantiasa menyertai keluarga. Menjelang hari esok, "
                f"izin mengingatkan jadwal iuran kas rutin kita sebesar *Rp{nd:,}*.\n\n"
                f"Pembayaran bisa melalui *Tunai* atau *Scan QRIS*.\n\n"
                f"Semoga setiap rupiahnya menjadi wasilah pembersih harta dan pengetuk pintu langit. Amin. 🤲✨"
            )
        elif s > 0:
            msg = (
                f"Assalamu’alaikum Bapak/Ibu *{n}*. 🌸\n\n"
                f"Mohon izin menyampaikan amanah catatan kas yang masih tertunda selama *{int(s)} bulan* "
                f"(Total *Rp{int(s*nd):,}*).\n\n"
                f"Kami mendoakan semoga Allah membukakan pintu rezeki dari arah yang tak disangka-sangka "
                f"agar Bapak/Ibu dapat menunaikannya via *Tunai/QRIS*.\n\n"
                f"Jazakumullah khairan atas perhatiannya. 😊🙏"
            )
        elif s == 0:
            msg = (
                f"Assalamu’alaikum Bapak/Ibu *{n}*. ✨\n\n"
                f"Alhamdulillah, administrasi kas Anda saat ini dalam status *LUNAS*.\n\n"
                f"Terima kasih atas kerja samanya dalam menjaga amanah grup ini. "
                f"Semoga Allah senantiasa melimpahkan rezeki dan berkah-Nya. Syukron jazilan. 🙏🤲"
            )
        else: # s < 0 (Lebih bayar)
            msg = (
                f"Masya Allah, Tabarakallah Bapak/Ibu *{n}*. 🌟\n\n"
                f"Jazakumullah khairan katsiran atas kedisiplinannya. Status kas Anda saat ini *LUNAS* "
                f"(Bahkan ada lebih bayar *{abs(int(s))} bulan*).\n\n"
                f"Semoga Allah menggantinya dengan kesehatan, kebahagiaan, dan keberkahan yang berlipat ganda "
                f"untuk keluarga. Amin. 🤲🕋"
            )
        webbrowser.open(f"https://wa.me/{wa_f}?text={urllib.parse.quote(msg)}")
    ...
    def kelola_transaksi_anggota(self, nama):
        ...
        for tr in reversed(self.transaksi):
            if tr['nama'] == nama:
                ...
                row.add_widget(Label(text=f"{tr['tgl']} | Rp{tr['jumlah']:,}", color=WARNA_TEKS_UTAMA, font_size='14sp'))
...
class KasApp(App):
    def build(self):
        sm = ScreenManager(); sm.add_widget(LoginScreen(name='login')); sm.add_widget(MainScreen(name='utama'))
        return sm

if __name__ == '__main__': KasApp().run()
