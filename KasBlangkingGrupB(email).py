...
# --- LIBRARY TAMBAHAN UNTUK EMAIL & BACKUP ---
import smtplib
import threading
...
class MainScreen(Screen):
    ...
    # --- FITUR BACKUP & RESTORE (Lokal & Email) ---
    def save_data(self):
        try:
            # Simpan File Lokal
            ...
            # BACKUP OTOMATIS KE EMAIL (FITUR BARU)
            self.kirim_email_backup(self.file_data)
            
        except Exception as e:
            print(f"Gagal Simpan: {e}")

    def kirim_email_backup(self, filepath):
        # Jalankan di thread berbeda agar UI tidak lag
        threading.Thread(target=self._eksekusi_email, args=(filepath,)).start()

    def _eksekusi_email(self, filepath):
        pengirim = "blangkinggrupb@gmail.com"
        password = "frli ubjt aaqh djdx" 
        penerima = "blangkinggrupb@gmail.com"

        msg = MIMEMultipart()
        msg['From'] = pengirim
        msg['To'] = penerima
        msg['Subject'] = f"AUTO-BACKUP KAS: {datetime.now().strftime('%d/%m/%Y %H:%M')}"

        body = "Terlampir backup otomatis data Kas Blangking terbaru dalam format JSON."
        ...
        try:
            ...
            server.quit()
            print("Backup email berhasil dikirim!")
        except Exception as e:
            print(f"Gagal mengirim email backup: {e}")
